  ...
  <div class="hello">
   ...
   <div class= "content">
        <div class="form-group">
          ...
          <select v-model="department" @click = filterdatas(department) type="text" class="form-control" id="department" placeholder="select a department">
            <option>admin</option>
...
export default {
  data: {
      items: [],
      department: ["admin", "support", "proyects", "contracts", "rrhh"]
    },
  mounted () {
  axios.get('http://localhost:3001/employees-list')
    ...
    .catch(error => {
      console.log(error)
        ...
        var columns = ['Departments', 'Previous', 'Current'],
          offset = 1;
  ...
  methods: {
    filterData: function (department) {
      console.log("Depart select-> ", department)
    } 
  }
}
